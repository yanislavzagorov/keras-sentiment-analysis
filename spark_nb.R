...
reviews_spark <- sdf_copy_to(sc, reviews, overwrite=TRUE)


# ---------//  ~~Building Models~~  //---------
nb_pipeline <- ml_pipeline(
  ...
  ml_naive_bayes(sc, label_col = "Sentiment", features_col = "vectorizer_output")
)


# ---------//  ~~Fitting & Predictions~~  //---------
partitions <- sdf_partition(reviews_spark, training=0.7, test=0.3, seed=123)
nb_model <- ml_fit(nb_pipeline, partitions$training)
nb_pred <- ml_transform(nb_model, partitions$test)
